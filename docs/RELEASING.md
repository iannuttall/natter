...
spctl --assess --type execute --verbose=4 dist/Dictation.app
xcrun stapler validate dist/Dictation.app
shasum -a 256 -c dist/Dictation-VERSION.zip.sha256
```
...
Before the first public build, replace the temporary product name, add a proper
app icon, decide the initial version, run the clean-account matrix above and
create the first notarization profile. Auto-update can wait until after the
first manually distributed build.
