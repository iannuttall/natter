    ...
    private var delivery: some View {
        VStack(alignment: .leading, spacing: Theme.Space.regular) {
            ...
            Text("Pace long input so coding agents do not collapse it into a paste block.")
                .font(.caption)
                .foregroundStyle(.secondary)
            Toggle("Stream into terminals while speaking", isOn: $store.terminalStreamsLive)
            Text("Types words into the terminal as you talk so you can re-read a long dictation before sending. Applies to Raw, Clean and Agent in terminal apps.")
                .font(.caption)
                .foregroundStyle(.secondary)
            Divider()
